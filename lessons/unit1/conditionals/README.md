...
The ```switch``` statement is a conditional that is most useful when the set of possible values we're comparing to is known. Swift's ```switch``` statement is more powerful than its predecessors so it can be used more widely. In many cases ```switch``` is more readable than the alternative. That alternative is almost always a series of ```if-then-else``` blocks.
